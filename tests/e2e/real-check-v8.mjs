...
// 给每幕一个有意义的标题
const actTitles = ['幕一 · 召唤与入局', '幕二上 · 调查推进', '幕二下 · 黑暗时刻', '幕三 · 真相与代价'];
...
// 给关键节点填 note
const nodeNotes = {
  opening_image: '黄昏的小镇渡口，林知夏戴口罩低头登岸。一只船尾褪色的红绳。',
  inciting_incident: '父亲遗物里的录音笔录下陈牧的声音。',
  catalyst: '父亲临终把"老周"的名字交给她。',
  midpoint: '雨夜水坝对峙，陈牧第一次承认见过妹妹。',
  crisis: '林知夏翻出当年自己签的"放弃寻找"声明 — 她也是共谋。',
  climax: '镇政府台阶公开播放录音 + 备忘。',
  resolution: '渡口黄昏，林知夏独自上船离开。'
};
...
const characters = [
  { name: '林知夏', story_role: 'protagonist',
...
for (let i = 0; i < rels.length; i++) {
  ...
  await fillField(page, 'relationship-field', 'status', r.status);
...
const plotSpecs = [
  // Act 1 — 7 张
  { actIdx: 0, title: '回到小镇',     summary: '林知夏因父亲病危回到阔别十年的小镇。',                                  conflict: '不愿见陈牧，被高调相迎。',                  change: '从抗拒转为被迫停留。' },
  { actIdx: 0, title: '医院走廊',     summary: '林知夏见弥留的父亲，父亲示意她回家找箱子最底的东西。',                  conflict: '父亲气息短话不清，旁边护士在场。',          change: '心生疑惑。' },
  { actIdx: 0, title: '父亲卧室',     summary: '深夜整理遗物，发现妹妹失踪当日的录音笔，最后一段录音里有陈牧的声音。', conflict: '录音笔老化，关键段噪声大。',                change: '怀疑陈牧。' },
  { actIdx: 0, title: '理发店传言',   summary: '阿珍口中陈牧近年的体面 + 一丝异样。',                                  conflict: '阿珍欲言又止。',                            change: '决心调查。' },
  { actIdx: 0, title: '父亲家访客',   summary: '陈牧主动送白菊登门邀晚饭。',                                          conflict: '表面寒暄，桌下试探。',                      change: '答应赴约。' },
  { actIdx: 0, title: '陈牧家晚饭',   summary: '三人桌下暗战，苏曼第一次起疑。',                                      conflict: '所有话不能直说。',                          change: '心照不宣。' },
  { actIdx: 0, title: '医院告别',     summary: '父亲临终把"老周"的名字交给她。',                                       conflict: '父亲只剩最后一口气，外面有人在等她。',      change: '父亲去世。' },
  // Act 2 上 — 6 张
  { actIdx: 1, title: '理发店深谈',   summary: '阿珍崩溃说看到陈牧带妹妹去水坝。',                                    conflict: '阿珍害怕被报复。',                          change: '确信。' },
  { actIdx: 1, title: '水边踩点',     summary: 'A 独自勘查当年现场，发现一处被冲刷的旧物。',                          conflict: '现场十年未动，线索模糊。',                  change: '心底动摇。' },
  { actIdx: 1, title: '镇政府远观',   summary: '远远看到陈牧风光发表演讲。',                                          conflict: '不能正面挑衅。',                            change: '愤怒压不住。' },
  { actIdx: 1, title: '找老周',       summary: 'A 在老周门口等了 3 小时。',                                          conflict: '老周关门。',                                change: '撬开一条缝。' },
  { actIdx: 1, title: '老周交备忘',   summary: '老周交出当年未上报的尸检备忘。',                                      conflict: '老周要求不提自己。',                        change: '握有铁证。' },
  { actIdx: 1, title: '夜里复印',     summary: 'A 复印备忘并把原件藏起。',                                            conflict: '没人能帮她保管。',                          change: '战术明确。' },
  // Act 2 下 — 5 张
  { actIdx: 2, title: '水坝雨夜对峙', summary: '雨夜把陈牧约到现场出录音笔。',                                        conflict: '陈牧不肯承认去向。',                        change: '陈牧承认见过。' },
  { actIdx: 2, title: '父亲遗物抽屉', summary: 'A 翻出当年自己签的"放弃寻找"声明 — 自我审判。',                       conflict: '她意识到自己也是共谋。',                    change: '羞耻、动摇。' },
  { actIdx: 2, title: '雨中独行',     summary: '失神时刻，她想过放弃。',                                              conflict: '内心的引诱。',                              change: '重新站起。' },
  { actIdx: 2, title: '苏曼来访',     summary: '苏曼悄悄上门请求 A 给所有人一个交代。',                              conflict: '苏曼也怕，但更怕女儿长大听到。',            change: '获得同盟。' },
  { actIdx: 2, title: '陈牧的恐惧',   summary: '陈牧家书房，他酗酒被苏曼撞见。',                                      conflict: 'B 不愿承认自己崩溃。',                      change: '裂痕清晰。' },
  // Act 3 — 3 张
  { actIdx: 3, title: '镇政府台阶',   summary: 'A 公开放出录音 + 备忘片段，陈牧当众崩溃。',                          conflict: '陈牧拥护者阻拦。',                          change: '颜面碎裂。' },
  { actIdx: 3, title: '警车前五分钟', summary: '陈牧被带走前承认意外细节。',                                          conflict: '原谅 vs 愤怒。',                            change: '放下愤怒。' },
  { actIdx: 3, title: '渡口离开',     summary: '与开场对位，她独自上船。',                                            conflict: '没有人来送。',                              change: '可以离开。' }
];

const createdCardIds = [];
for (let i = 0; i < plotSpecs.length; i++) {
  const c = plotSpecs[i];
  if (i % 5 === 0) console.log(`  [${i + 1}/${plotSpecs.length}] ${c.title}`);
  await clickById(page, 'add-plot-card'); await w(280);
  // 取新加的卡 id（最后一张）
  ...
  // 按幕分配
  if (actIds[c.actIdx]) {
    await fillField(page, 'plot-position-field', 'act_id', actIds[c.actIdx]);
  }
  // 锁定状态
...
const dl = await dp;
const fp = path.join(SHOTS, '小镇七十二小时-v8.fountain');
