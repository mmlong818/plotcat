// 第 2 轮：白桦树下 + 晨星 2049 两部长片
process.env.NO_PROXY = '*';
process.env.no_proxy = '*';

import { BAIHUA_SCENES } from './generators/07-baihua.mjs';
import { CHENXING_SCENES } from './generators/10-chenxing.mjs';

const BASE = 'http://127.0.0.1:4173';

const TARGETS = [
  { idx: 7, label: '白桦树下', slug: '07-xiaoyuan-qingchun', projectId: 'project_mpo78ngx_slq32f', scenes: BAIHUA_SCENES },
  { idx: 10, label: '晨星 2049', slug: '10-kehuan-ai', projectId: 'project_mpo79am3_tkcfra', scenes: CHENXING_SCENES },
];
...
async function main() {
  ...
  console.log('\n--- 总结 ---');
  for (const r of results) console.log(`[${r.idx}/10] ${r.label} | 长片 | ${r.sceneCount} 场 | ${r.total} 字 | 单场最低 ${r.min}`);
}
